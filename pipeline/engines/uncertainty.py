...
def tta_mc_inference(backbone, decoder, image_tensor: torch.Tensor,
                     n_passes: int = MC_DROPOUT_PASSES) -> dict:
    """
    Run MC Dropout inference with Test-Time Augmentation (TTA).
    Uses a 70/30 weighted fusion of Original and Horizontally Flipped inferences
    to safely boost confidence without blurring sharp mask edges.
    """
    # 1. Original Pass (Epistemic Uncertainty via N passes)
    info_orig = mc_inference(backbone, decoder, image_tensor, n_passes=n_passes)

    # 2. Flipped Pass (Single deterministic pass)
    # Using n_passes=1 saves compute while still giving us the spatial geometry boost
    image_flipped = torch.flip(image_tensor, dims=[-1])
    info_flip = mc_inference(backbone, decoder, image_flipped, n_passes=1)

    # Flip the spatial probabilities back
    info_flip["damage_probs"]  = torch.flip(info_flip["damage_probs"], dims=[-1])
    info_flip["anatomy_probs"] = torch.flip(info_flip["anatomy_probs"], dims=[-1])

    # 3. Safe Weighted Fusion (70% Original, 30% Flipped)
    w_orig, w_flip = 0.7, 0.3
    
    merged = {}
    merged["damage_probs"]  = info_orig["damage_probs"] * w_orig + info_flip["damage_probs"] * w_flip
    merged["anatomy_probs"] = info_orig["anatomy_probs"] * w_orig + info_flip["anatomy_probs"] * w_flip

    # Retain the uncertainty maps and routing logic from the original full N-pass run
    merged["damage_uncertainty_map"]  = info_orig["damage_uncertainty_map"]
    merged["anatomy_uncertainty_map"] = info_orig["anatomy_uncertainty_map"]
    merged["damage_uncertainty"]      = info_orig["damage_uncertainty"]
    merged["anatomy_uncertainty"]     = info_orig["anatomy_uncertainty"]
    merged["overall_uncertainty"]     = info_orig["overall_uncertainty"]
    merged["routing"]                 = info_orig["routing"]
